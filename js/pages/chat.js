    ...
    openChat(targetUid, targetName) {
        ...
        unsubscribeMessages = onSnapshot(q, (snapshot) => {
            ...
            snapshot.forEach(msgDoc => {
                ...
                if (isMe) {
                    const receiptIcon = data.read ? `<span style="color: #00e6e6; margin-left: 4px; vertical-align: middle;">${Icons.CheckDouble()}</span>` : `<span style="color: rgba(0,0,0,0); margin-left: 4px; vertical-align: middle;">${Icons.Check()}</span>`;
                    html += `
